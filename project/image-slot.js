...
(() => {
  ...
  async function toDataUrl(file, targetW) {
    // Cap the longest side at MAX_DIM only — never at the slot's rendered
    // width. A collapsed/hidden slot (0px, or in an inactive tab) used to
    // bake in a tiny image; now the stored bytes always keep the source
    // resolution up to MAX_DIM regardless of layout at drop time.
    const bitmap = await createImageBitmap(file);
    try {
      const longest = Math.max(bitmap.width, bitmap.height);
      const scale = Math.min(1, MAX_DIM / longest);
      if (scale === 1) {
        // Source already within the cap: keep the ORIGINAL bytes untouched
        // (no canvas re-encode, no recompression, no smoothing softness).
  ...
  const stylesheet =
    ...
    // Branded loading tile: shown while an image is decoding / being
    // re-encoded, so a slow product shot never flashes an empty frame.
    '.loading{position:absolute;inset:0;display:none;align-items:center;' +
    '  justify-content:center;pointer-events:none;' +
    '  background:#f4f4f2 url("image-loading.png") center/cover no-repeat}' +
    ':host([data-swapping]) .loading{display:flex}' +
    ':host([data-loading]) .loading{display:flex}' +
